...
REVIEW_PAGE_PATH = SCRIPTS / "nova_nothingbuta_local_preview_review_page.py"
...
def backup_file(path: Path) -> Path:
    ...
    backup_path = path.with_name(f"{path.name}.bak-preview-review-autopilot-{stamp}")
...
def patch_autopilot() -> Path:
    ...
    if not REVIEW_PAGE_PATH.exists():
        logging.error("Missing preview review page script: %s", REVIEW_PAGE_PATH)
        raise SystemExit(1)

    source = AUTOPILOT_PATH.read_text(encoding="utf-8")

    review_line = '    ["python", str(SCRIPTS / "nova_nothingbuta_local_preview_review_page.py")],'

    if review_line in source:
        print("Preview review page is already in safe autopilot.")
        return AUTOPILOT_PATH

    anchor = '    ["python", str(SCRIPTS / "nova_nothingbuta_release_freeze_packet.py")],'

    if anchor not in source:
        logging.error("Could not find release freeze command anchor in autopilot.")
        raise SystemExit(1)
...
def main() -> None:
    ...
    print("NOTHINGBUTA PREVIEW REVIEW AUTOPILOT PATCH")
