...
# Objective:
# Apply NothingButA Optimization Batch 002 to the mortgage payment calculator.
#
# This script DOES:
...
logger = logging.getLogger("nothingbuta_optimization_batch_002")
...
TARGET_SLUG = "mortgage-payment-calculator"
...
BATCH_ID = "nothingbuta-optimization-batch-002"
CONTENT_MARKER = "nothingbuta-optimization-batch-002-content"
SCHEMA_MARKER = "nothingbuta-optimization-batch-002-schema"
ROOT_DROPDOWN_OPTION = '<option value="../">All tools home</option>'

JSON_OUT = DATA_DIR / "nothingbuta_optimization_batch_002.json"
MD_OUT = REPORTS_DIR / "nothingbuta-optimization-batch-002.md"

NEW_META_DESCRIPTION = (
    "Estimate a monthly mortgage payment using loan amount, interest rate, loan term, "
    "taxes, insurance, and other housing costs before you shop."
)

STRUCTURED_DATA = {
    "@context": "https://schema.org",
    "@type": "WebApplication",
    "name": "NothingButA Mortgage Payment Calculator",
    "url": "https://cbw29512.github.io/nothingbuta/mortgage-payment-calculator/",
...
def backup_target() -> Path:
    try:
        ...
        backup_path = BACKUP_ROOT / f"{TARGET_SLUG}-before-optimization-002-{stamp}.html"
...
def build_content_section() -> str:
    return f"""
<section class="nba-growth-section" id="{CONTENT_MARKER}" aria-labelledby="mortgage-growth-help">
  <style>
    .nba-growth-section {{
  ...
  <h2 id="mortgage-growth-help">Make the mortgage estimate useful</h2>

  <h3>How to use this calculator</h3>
  <p>
    Enter the loan amount, interest rate, loan length, and any extra monthly housing costs.
    The calculator gives you an estimated monthly payment so you can compare homes,
    loan scenarios, or budget ranges before making a bigger decision.
  </p>

  <h3>What the result means</h3>
  <p>
    Treat the result as a planning estimate, not a lender quote. Your final payment can change
    based on taxes, insurance, HOA fees, private mortgage insurance, escrow rules, credit profile,
    down payment, and lender terms.
  </p>

  <h3>Useful next checks</h3>
  <p>
    After estimating a mortgage payment, compare it with your monthly income, emergency fund,
    and other debts. A payment that looks affordable alone may feel different once maintenance,
    utilities, and savings goals are included.
  </p>
</section>
""".strip()
...
def write_reports(proof: dict) -> None:
    try:
        ...
        lines = [
            "# NothingButA Optimization Batch 002",
            "",
...
def main() -> int:
    try:
        ...
        if content_changed:
            changes.append("added_helpful_copy_section")
        ...
        print("NOTHINGBUTA OPTIMIZATION BATCH 002:", proof["status"])
        ...
        print("NOTHINGBUTA OPTIMIZATION BATCH 002: FAILED")
        print(str(exc))
        return 1
